# Install and integrate Passbolt MCP

## Install a release package

Install [uv](https://docs.astral.sh/uv/getting-started/installation/) and download the
wheel `zai_passbolt_mcp-0.3.0-py3-none-any.whl` plus `SHA256SUMS.txt` from [v0.3.0](https://github.com/zai-one/passbolt-mcp/releases/tag/v0.3.0).
...
```sh
uv tool install "zai-passbolt-mcp[standalone] @ https://github.com/zai-one/passbolt-mcp/releases/download/v0.3.0/zai_passbolt_mcp-0.3.0-py3-none-any.whl"
passbolt-mcp-setup --directory /absolute/path/to/my-passbolt-settings
passbolt-mcp --config /absolute/path/to/my-passbolt-settings/mcp.local.json --check-config
```
...
Windows accepts paths such as `C:/Users/you/mcp/passbolt`; quote paths containing
...
```sh
passbolt-mcp-setup --directory /absolute/path/to/my-passbolt-settings --client-only
passbolt-mcp-setup --directory /absolute/path/to/my-passbolt-settings --client-only --format toml
```
...
Install GnuPG (Linux package `gnupg`, Windows Gpg4win) and ensure `gpg` is on PATH.
Git for Windows GnuPG is supported with its companion `cygpath.exe` and MSYS runtime;
keep those binaries together.
Prepare an owner-private copy of `examples/passbolt.env.example`, replacing all placeholders.
Import your service user's key into a dedicated GPG home; configure the private passphrase
file, server/metadata fingerprints, token state path and `PASSBOLT_SINK_CONFIG_FILE`.
Use `examples/policy-sinks.json` as the policy reference and bind local-operator to a policy
label that exists in that file (`owner_all` is generated by the wizard; narrow the example policy to your intended resources).
These values come from your own vault administrator; the project cannot infer them.
Keep `PASSBOLT_WRITE_ENABLED` and `PASSBOLT_USE_ENABLED` false for initial read-only setup.
Local doctor checks GnuPG availability and the outer config; live vault auth/decryption
is inspected through passbolt_status after the operator supplies and reviews the configuration.
...
`C:/Users/you/mcp/passbolt-mcp`). Give the MCP client this stdio configuration:
...
```sh
uv run --frozen --extra standalone passbolt-mcp --config mcp.local.json --transport http --check-config
uv run --frozen --extra standalone passbolt-mcp --config mcp.local.json --transport http --host 127.0.0.1
```

Configure `PASSBOLT_MCP_PUBLIC_KEY_FILE` with an RSA verification key and issue
